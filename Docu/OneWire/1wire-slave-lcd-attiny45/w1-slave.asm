 ...
 * 
 * All functions needed for a 1wire slave. The 1wire ID is obtained by a
 * connected DS2401 "silicon serial number" at PB4. All functions are interrupt
 * driven. All ROM-commands are covered by this program. The 256 byte internal
 * eeprom can be accessed with the same commands like DS2433 1wire EEPROM.
 * 
 * For all other DEVICE-commands the function "w1_data_other" is called with
 * some useful variables set. There the user can implement own 1wire DEVICE-
 * commands.
 * 
 * ----------------------------------------------------------------------------------
 * 
...
w1_slave_init_ram:
		...
		rcall	fd_uart_init				;init full duplex uart
		...
		ldi		ZL,LOW(2*str_init)			;output init string
		ldi		ZH,HIGH(2*str_init)
		rcall	fd_uart_put_PGM_str

		mov		tmp0,w1_stat				;any error flag?
		andi	tmp0,~(1<<W1_SB0|1<<W1_SB1)	;except W1_SB0 W1_SB1
		breq	w1_ok
		ldi		ZL,LOW(2*str_error)
		ldi		ZH,HIGH(2*str_error)
		rcall	fd_uart_put_PGM_str
		mov		tx_data,w1_stat				;output error code
		rcall	fd_uart_putchar_hex
		ldi		tmp0,' '
		mov		tx_data,tmp0
		rcall	fd_uart_putchar_wait
		rcall	fd_uart_put_enter

w1_ok:
		ldi		tmp0,0x01					;rom type 0x01 -> serial number
...
w1_id_copy_loop:
		...
		rjmp	w1_id_ok
w1_id_not_found:
		ldi		ZL,LOW(2*str_noid)			;output error, no id-chip
		ldi		ZH,HIGH(2*str_noid)
		rcall	fd_uart_put_PGM_str
...
w1_id_not_found_loop:
		lpm		tx_data,Z+					;load default id
		st		X+,tx_data					;and store it
		dec		tmp1
		brne	w1_id_not_found_loop
		lds		tmp0,w1_id					;load chip id
		rcall	w1_id_set_chip				;set chip id, compute and store crc
		rjmp	w1_id_not_found_out
w1_id_ok:
		ldi		ZL,LOW(2*str_id)			;output error, no id-chip
		ldi		ZH,HIGH(2*str_id)
		rcall	fd_uart_put_PGM_str
w1_id_not_found_out:
		ldi		XL,LOW(w1_id)				;init data pointer to id store
		ldi		XH,HIGH(w1_id)
		ldi		tmp1,8						;loop counter, send 8 bytes
w1_id_ok_loop:
		ldi		tmp0,' '
		mov		tx_data,tmp0
		rcall	fd_uart_putchar_wait
...
id_default:
		.db		0x72,0x6c,0x33,0x01,0x00,0x00,0x00,0x00
str_error:
		.db		"w1-error: ",0,0
str_id:
		.db		"w1-id:",0,0
