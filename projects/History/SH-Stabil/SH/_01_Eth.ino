              ...
              if (!parseHexChar(ethInBuffer[3], digit)) {
                ...
                if( AddrByteIndex != 7) {if (ethLogLevel > 0) Debug.println(F("\neth: owAddr: Addresslänge stimmt nicht!")); }
                  ...
                  for (AddrByteIndex=0; AddrByteIndex < 8; AddrByteIndex++) {
                    if (EEPROM.read(owArrayIndex * 8 + AddrByteIndex) != Addr[AddrByteIndex]){
                      if (ethLogLevel > 0) {
                        Debug.print(F("\neth:     Schreibe EEPROM(")); 
                        Debug.print( owArrayIndex * 8 + AddrByteIndex); 
                        Debug.print(F(", ")); 
                        Debug.print( Addr[AddrByteIndex], HEX); 
                        Debug.println(F(")")); 
                      }
                      EEPROM.write(owArrayIndex * 8 + AddrByteIndex, Addr[AddrByteIndex]);
                    }
                  }
