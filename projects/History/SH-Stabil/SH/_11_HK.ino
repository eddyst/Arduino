#define HKMischerRichtungPin 99
#define HKMischerBewegenPin 99
...
void   HKDoEvents() {
  ...
  if (Values[_HKVorlaufTempGemischt].ValueX10 != ValueUnknown) { //Wenn die Vorlauftemperatur nicht gesetzt wurde können wir nichts regeln
    if (Values[_HKVorlaufTempGemischt].ValueX10 > 600) {  // Zudrehen!!!!
      ...
      if (millis() - lastMove > 20000) {                         // Wenn min ? Sekunden seit letzter Bewegung
        ...
        if (Values[_HKVorlaufTempGemischt].ValueX10 > HKSollTempVorgabe + 5 ) {         // Bei 5 C° Überschreitung
        ...
        else if (Values[_HKVorlaufTempGemischt].ValueX10 < HKSollTempVorgabe - 2 ){
