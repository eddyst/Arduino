...
#define AddressCount 22
uint8_t Address_00[] = { 0x25, 0x00, 0x16};
uint8_t Address_01[] = { 0x55, 0x5A, 0x02};
uint8_t Address_02[] = { 0xA3, 0x93, 0x02};
uint8_t Address_03[] = { 0x08, 0x10, 0x02};
uint8_t Address_04[] = { 0x25, 0x44, 0x02};
uint8_t Address_05[] = { 0x08, 0x08, 0x02};
uint8_t Address_06[] = { 0x08, 0x12, 0x02};
//uint8_t Address_[] = { 0x08, 0x14, 0x02}; = immer 20
uint8_t Address_08[] = { 0x55, 0x25, 0x02};
uint8_t Address_09[] = { 0x55, 0x27, 0x02};
uint8_t Address_10[] = { 0x0A, 0x10, 0x01};
uint8_t Address_11[] = { 0x08, 0x8A, 0x04};
uint8_t Address_12[] = { 0x08, 0x86, 0x04};
uint8_t Address_13[] = { 0x23, 0x23, 0x01};
uint8_t Address_14[] = { 0x0A, 0x82, 0x01};
uint8_t Address_15[] = { 0x23, 0x02, 0x01};
uint8_t Address_16[] = { 0x23, 0x03, 0x01};
uint8_t Address_17[] = { 0x65, 0x13, 0x01};
uint8_t Address_18[] = { 0x76, 0x60, 0x02};
uint8_t Address_19[] = { 0x76, 0x63, 0x02};
uint8_t Address_20[] = { 0xA3, 0x8F, 0x02};
//uint8_t Address_21[] = { 0x65, 0x15, 0x01};

const uint8_t *addressTable[AddressCount] = { Address_00, Address_01, Address_02, Address_03, Address_04, Address_05, Address_06, Address_07, Address_08, Address_09, 
                                              Address_10, Address_11, Address_12, Address_13, Address_14, Address_15, Address_16, Address_17, Address_18, Address_19,
                                              Address_20, Address_21};
uint8_t lastValue[61];  //61 = Summe der 3. Felder aus Adresse_??                                 
...
void setup() {
  Log.print( F( "g"));  Log.print( freeMemory());
  pinMode( 2,OUTPUT);
...
//  led05Empfangen.doEvents();
  ...
  if ( millis() - TimerEthSend > 15000) {
    Log.print( F( "w")); Log.print( freeMemory());
...
//boolean BigLog = false;
  ...
  case WaitForConnectionOffer:
    if ( inputBuffer.getSize()) {
      ...
      if ( B == 0x05) {
        ...
        optoWrite( ( uint8_t[]){ 0x16, 0x00, 0x00}, 3);
        ledWaitForAnswer.blinkBegin( 60000);
        break;
      }
      else {
         Log.print( F( "l")); Log.print( B, HEX);
      }
    }
    break;
  case NotInitialized:
  ...
  case Initialized:
    if ( Log.length() < 50) {
      ...
      Log.print( F( "o")); Log.print( String( run));
...
//    Auswertung: 0x20B8 = V333MW1
      optoWrite( ( uint8_t[]){ 0x41, 0x05, 0x00, 0x01, addressTable[run][0], addressTable[run][1], addressTable[run][2], 
                             ( 6 + addressTable[run][0] + addressTable[run][1] + addressTable[run][2]) % 256}, 8);
 ...
 static uint8_t dataLength;
      ...
      case 0: 
        if ( B == 0x06) {
          ...
          Log.print( F( "P")); Log.print( B, HEX); //          LogCurrentDS+="P"; LogCurrentDS += String( B, HEX); BigLog = true;
          pStatus = None;
        }
        break; 
      case 1: 
      ...
      case 5:
        ...
        if ( addressByte1 != addressTable[run][0]){     //Das ist nicht die Adresse nach der ich gefragt habe
          Log.print( F( "T")); Log.print( run); Log.print( F( "_")); Log.print( addressTable[run][0], HEX);
          pStatus = None;
        }
        break;
      case 6:
        ...
        if ( addressByte2 != addressTable[run][1]){     //Das ist nicht die Adresse nach der ich gefragt habe
          Log.print( F( "U")); Log.print( run); Log.print( F( "_")); Log.print( addressTable[run][1], HEX);
          pStatus = None;
        }
       break;
      case 7:
        ...
        if ( B != addressTable[run][2]){     //Bitte klären warum die erwartete Anzahl Bytes nicht stimmt
          Log.print( F( "R")); Log.print( run); Log.print( F( "_")); Log.print( addressTable[run][2], HEX);
          pStatus = None;
        }
        break;
      default:
