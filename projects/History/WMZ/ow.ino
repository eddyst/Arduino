...
#define owStateREAD                   7
  static uint8_t owState = owStateUPDATE;
  ...
  case owStateSEARCH:
    if ( !owBus.search( owAddr)) {
      if ( Values[_Vorlauf].Read && Values[_Ruecklauf].Read) {
        if ( Values[_Vorlauf].Changed || Values[_Ruecklauf].Changed) {
          setDeltaT( Values[_Vorlauf].ValueX10 - Values[_Ruecklauf].ValueX10);
          Values[_Vorlauf].Changed = 0;
          Values[_Ruecklauf].Changed = 0;
        }
        Values[_Vorlauf].Read = 0;
        Values[_Ruecklauf].Read = 0;
      }
