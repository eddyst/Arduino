        ...
        if (c == '>') {
          ...
          if (recording == 0) {
            ...
            if (ethInBufferCount > 7
              ...
              else if (ethInBuffer[4] == 'K' && ethInBuffer[5] == 'p') {
                if ( setValue( _HKKp, Value) && hkLogLevel > 1) {
                  Debug.print   ( F("HK: HKKp Zugewiesen: "));
                  Debug.println   ( Values[_HKKp].ValueX10);
                  myPIDChange ( Values[_HKKp].ValueX10, Values[_HKKi].ValueX10, Values[_HKKd].ValueX10);
                }
              }
              else if (ethInBuffer[4] == 'K' && ethInBuffer[5] == 'i') {
                if ( setValue( _HKKi, Value) && hkLogLevel > 1) {
                  Debug.print   ( F("HK: _HKKi Zugewiesen: "));
                  Debug.println   ( Values[_HKKi].ValueX10);
                  myPIDChange ( Values[_HKKp].ValueX10, Values[_HKKi].ValueX10, Values[_HKKd].ValueX10);
                }
              }
              else if (ethInBuffer[4] == 'K' && ethInBuffer[5] == 'd') {
                if ( setValue( _HKKd, Value) && hkLogLevel > 1) {
                  Debug.print   ( F("HK: _HKKd Zugewiesen: "));
                  Debug.println   ( Values[_HKKd].ValueX10);
                  myPIDChange ( Values[_HKKp].ValueX10, Values[_HKKi].ValueX10, Values[_HKKd].ValueX10);
                }
              }
            }
    ...
    switch (Task) {
      ...
      case 1:
        ...
        if (millis() - lastSollSync > SollSyncIntervall) { //jede Stunde Addressen neu abfragen
          //Sende: { "<SollHK=".Value("HKSollTempVorgabe").">" }
          ...
          switch ( Value++) {
            ...
            case 1:
              ...
              break;
            case 2:
              if (ethLogLevel > 1) Debug.println( F( "eth: OUT: { sprintf( \"<SollKp=%.0f>\", Value(\"HKKp\") * 10)}"));
              client.println( F( "{ sprintf( \"<SollKp=%.0f>\", Value(\"HKKp\") * 10)}"));
              break;
            case 3:
              if (ethLogLevel > 1) Debug.println( F( "eth: OUT: { sprintf( \"<SollKi=%.0f>\", Value(\"HKKi\") * 10)}"));
              client.println( F( "{ sprintf( \"<SollKi=%.0f>\", Value(\"HKKi\") * 10)}"));
              break;
            case 4:
              if (ethLogLevel > 1) Debug.println( F( "eth: OUT: { sprintf( \"<SollKd=%.0f>\", Value(\"HKKd\") * 10)}"));
              client.println( F( "{ sprintf( \"<SollKd=%.0f>\", Value(\"HKKd\") * 10)}"));
              break;
            default:
              lastSollSync = millis();
              Task++;
