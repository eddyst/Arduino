...
uint8_t vitoAngeforderteBetriebsart = 255;

void vitoDoEvents () {
  Vito.doEvents();
  if (Vito.awaitingCommand()) {
    if (vitoAngeforderteBetriebsart != 255) { 
      Vito.beginWriteValue(0x23, 0x23, vitoAngeforderteBetriebsart);
      vitoAngeforderteBetriebsart = 255;
    } 
    else {
      ...
      if (VitoAdr > 8) {
        ...
        tmpUint16_1 = millis();
        if ( tmpUint16_1 - LastRead > 10000) { // Nur alle ? Sek die Abfrage gegen die Heizung starten
          VitoAdr = 0;               // Es wird ja vor der Auswertung dazugezählt
          LastRead=tmpUint16_1;
        }
      } 
      else
      {
